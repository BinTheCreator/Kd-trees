...
public class BrutePointST<Value> implements PointST<Value> {
    private RedBlackBinarySearchTreeST<Point2D, Value> bst; // An underlying data structure to
    // store the 2d points (keys) and their corresponding values
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        LinkedQueue<Point2D> d = new LinkedQueue<>();
    ...
    public Iterable<Point2D> nearest(Point2D p, int k) {
        ...
        MinPQ<Point2D> pq = new MinPQ<>(size(), p.distanceToOrder());
        for (Point2D b : bst.keys()) {
            if (b.equals(p)) {
                continue;
            }
            pq.insert(b);
